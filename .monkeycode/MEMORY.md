...
[Project Knowledge Summary]
- Date: 2026-08-13
- Context: Discovered by Agent while diagnosing why cfip-lite-mini found no IPs on Windows while yx-tools did
...
- Instructions:
  ...
  - cfip-lite-mini 已默认启用 `-http`（HTTPing）模式并用 HEAD 请求（完整 HTTP 交换含 TLS 握手与服务端响应，不传 body），`-http=false` 可回退旧 GET 模式。
